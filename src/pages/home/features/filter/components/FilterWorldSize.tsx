...
export default function FilterWorldSize({ filterResource, groupIds }: Props) {
	...
	return (
		<FilterOption
			...
			groupIds={groupIds}
			label={ReactDOMServer.renderToStaticMarkup(t("Filter World Size"))}
		>
			<span className="bm-span">{t("World Size")}</span>
			...
			<Select options={filterWorldSettings.map(settings => { return { option: settings + "%", value: settings } })} value={filterResource.size} onChange={e => filtersDispatch({
				type: "update",
				filterId: filterResource.id,
				groupIds,
				updater(draft: Filter.FilterWorldSize) {
